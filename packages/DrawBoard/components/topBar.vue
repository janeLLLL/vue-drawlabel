<template>
  <div class="bar">
    ...
    <el-tooltip effect="dark" content="删除标注" placement="bottom">
      <el-popconfirm
    ...
    <!-- v-if="this.isFocus && resultData.length === 0" -->
    <el-tooltip
      effect="dark"
      content="标注"
      placement="bottom"
      v-if="isFocus"
    >
      <i class="el-icon-news" @click="changeEvent('rectangle')"></i>
    </el-tooltip>

    <div class="status" v-if="this.currentStatus === 'DRAWING'">
      当前状态：标注
    </div>
    <div class="status" v-if="this.currentStatus === 'MOVING'">
      当前状态：移动
    </div>
    <div class="status" v-if="this.currentStatus === 'UPDATING'">
      当前状态：修改
    </div>
    <div class="status" v-if="this.currentStatus === 'DEFAULT'">
      当前状态：默认
    </div>
    <!-- <el-button
      size="mini"
